...
$(document).ready(function(){
	...
	if("${searchWord}" != ""){
	 ...
	 $("#sizePerPage").val('${sizePerPage}');
		...
		$("#sizePerPage").bind("change", function(){
			...
			frm.method = "GET";
			frm.action="<%= ctxPath %>/admin/productAdd.army";
			frm.submit();
		});
	...
	$(".add").bind("click", function(){
		var productno = $(this).val().trim();
		
		var count = $(this).parent().prev().find('input').val().trim();
		
		location.href="<%=ctxPath%>/admin/prodOneAdd.army?productno="+productno+"&count="+count;
	...
	function goSearch(){
		...
		var frm = document.prodFrm;
		frm.method = "GET";
		frm.action = "/SemiProject/admin/productAdd.army";
		frm.submit();
	}
	
	function addProduct(productno){
		
		location.href="<%=ctxPath%>/admin/prodOneAdd.army?productno="+productno;
...
<c:if test="${not empty requestScope.prodList}">
	<c:forEach var="prodvo" items="${prodList}" varStatus="status">
	<tr>
		...
		<td id="pstock">${prodvo.pstock }<br>
			<input type="number" id="addCount${status.count }" class="addCount" value="0" />
		</td>
		<td><button type="button" class="add" value="${prodvo.productno }">추가</button></td>
	</tr>
	</c:forEach>
</c:if>
